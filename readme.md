...
> Володимира. Значна переробка або перейменування не змінюють походження
> upstream-коду. Якщо конкретний фрагмент неможливо впевнено відділити від
> upstream, до нього зберігаються первісні умови до перевірки provenance в
> історії Git. Vendored `chess.js` має окрему BSD-ліцензію в
> [`app/web/vendor/chess.js.LICENSE`](app/web/vendor/chess.js.LICENSE).
...
## Як це працює (пояснення для людини)

### Нейронна мережа

Одна мережа (`ChessResNet`, `src/chess_zero/agent/torch_model.py`) робить
одразу дві речі з однієї й тієї ж позиції на дошці:

- **Policy (політика)** — наскільки хорошим виглядає кожен з 1968 можливих
  ходів у цій позиції (по суті "куди варто подивитись").
- **Value (оцінка)** — одне число від -1 (програш) до +1 (перемога): наскільки
  ця позиція взагалі хороша для того, хто ходить.

Усередині: дошка кодується в 18 "шарів" (число фігур × колір + додаткова
інформація на кшталт права на рокіровку), проганяється через 7
resnet-блоків (256 фільтрів кожен — та сама архітектура, що й в оригіналі
AlphaZero, тільки менша), і на виході розділяється на дві "голови" — policy
й value. Мережа сама по собі **нічого не рахує наперед** — це чиста
інтуїція за один прохід, як шахіст "з першого погляду" оцінює позицію.

### MCTS — як рушій насправді обирає хід

Саму лише інтуїцію мережі рушій не використовує напряму — навколо неї
працює **MCTS** (Monte Carlo Tree Search, `player_chess_torch.py`):
рушій подумки програє `N` варіантів наперед (`N` = складність, див.
нижче), на кожному кроці питаючи мережу "що тут добре", і накопичує
статистику відвідувань кожного ходу. Чим частіше якийсь хід виявляється
частиною хороших варіантів — тим більше "відвідувань" (`n`) він отримує.
Фінальний хід обирається за цією статистикою, не напряму за policy
мережі. Саме цю внутрішню статистику показує **теплокарта** на дошці
після кожного ходу рушія — кольором позначені клітинки, куди рушій
"дивився" найбільше.

### Цикл тренування: self-play → train → evaluate → promote

Кнопка "Тренування (1 цикл)" запускає один прогін чотирьох кроків
поспіль (`pipeline_torch.run_cycle`):

1. **Self-play** — поточна найкраща модель грає сама з собою кілька
   партій, записуючи кожну позицію разом з тим, куди MCTS "дивився"
   (policy) і як партія зрештою закінчилась (value).
2. **Train** — на цих щойно згенерованих партіях модель робить один
   прохід навчання (кандидат — окрема копія, поточна найкраща модель не
   чіпається, поки кандидат не доведе, що він кращий).
3. **Evaluate** — кандидат грає кілька "арена"-партій проти поточної
   найкращої моделі.
4. **Promote (чи ні)** — якщо кандидат виграв **≥55%** арена-партій, він
   стає новою найкращою моделлю (`model_best.pt`) і живий рушій одразу
   підхоплює нові ваги (hot-reload). Якщо ні — кандидат просто
   відкидається, найкраща модель лишається та сама.

Кожен прогін — і успішний, і ні — записується в **журнал поколінь**
(див. "Історія поколінь моделі" нижче), тож видно не лише "скільки разів
модель покращилась", а й скільки спроб не вдалось.

**Чесно:** один цикл — це лише кілька партій і одна епоха навчання,
навмисно маленький масштаб для перевірки, що весь механізм працює
коректно. Щоб модель реально стала помітно сильнішою за початкову
`model_best_weight.h5`, потрібні тисячі партій і багато циклів поспіль
(довгий, окремий прогін, не одна кнопка) — див.
[`docs/development-plan-uk.md`](docs/development-plan-uk.md).

### Звідки взялась початкова модель

`model_best_weight.h5` (початкові ваги, що йдуть разом з репозиторієм) —
це НЕ продукт self-play цього репо. Її натренував оригінальний автор
проєкту 2017-2018 року методом supervised learning на ~10 000 реальних
партій людей (деталі — розділ "About" нижче в цьому ж файлі, це
історична, ще Keras-епохи документація). Принцип "нуль зовнішніх знань"
(без дистиляції від Stockfish) стосується того, що робить **цей**
репозиторій зараз — self-play/тренувального циклу вище, а не походження
самого стартового чекпоінта.

**Якщо хочеш справжній нуль з самого початку** (без жодного supervised
learning на людських партіях, зовсім не той шлях, яким пройшов
оригінальний автор) — кнопка "Тренувати з нуля" нижче замінює модель
на випадкові, ненавчені ваги й повністю очищає журнал поколінь (старі
записи все одно стали б беззмістовними проти нової бази). Це
незворотно й вимагає підтвердження — після цього модель грає майже
навмання, поки self-play/тренування не накопичать реальний прогрес
(тисячі партій і циклів, не один запуск, те саме чесне застереження,
що й вище).

### Кнопки в GUI — що робить кожна

| Кнопка | Що робить |
|---|---|
| **Білі / Чорні** | яким кольором грає людина; змінюється лише коли нічого не виконується |
| **Легко / Середньо / Важко** | кількість MCTS-симуляцій на хід рушія: 10 / 50 / 200 — більше = сильніше й повільніше |
| **Нова партія** | скидає дошку, надсилає `ucinewgame`; якщо людина грає чорними — рушій одразу робить перший хід |
| **Скасувати хід** | відкочує останню пару ходів (людина+рушій) локально |
| **Здатись** | завершує партію як програш людини; партія зберігається (див. нижче) |
| **Дивитись самогру** | рушій грає сам із собою наживо, ходи стрімляться на дошку; повторне натискання зупиняє |
| **Тренування (1 цикл)** | запускає цикл вище; **Escape** під час тренування не скасовує його — лише відпускає GUI, цикл дограє у фоні (індикатор під основним статусом покаже, коли завершиться) |
| **Перезавантажити модель** | hot-reload: підхоплює найновіший `model_best.pt` без перезапуску всього рушія |
| **Історія поколінь моделі** | показує кожен цикл тренування (не лише успішні): номер, win-rate, промоутнутий чи ні, коли |
| **Тренувати з нуля (скинути модель)** | незворотно: заміняє модель на випадкові ваги, очищає журнал поколінь; запитує підтвердження; відмовляє під час активного тренування/самогри |

**Escape** під час ходу рушія чи самогри (де немає штатної зупинки) —
примусово перезапускає внутрішній процес рушія; поточний розрахунок
губиться, зате GUI негайно розблоковується.

### Де що зберігається

| Що | Де | Комітиться в git? |
|---|---|---|
| Початкові ваги (baseline) | `data/model/model_best_weight.h5` | так |
| Поточна найкраща модель (після тренувань) | `data/model_torch/model_best.pt` | ні (`.gitignore`) |
| Кожен цикл тренування (журнал) | `data/model_torch/next_generation/model_cycle{N}.json` | ні |
| Ваги промоутнутих циклів | `data/model_torch/next_generation/model_cycle{N}.pt` | ні |
| Партії self-play | `data/play_data_torch/` | ні |
| **Партії проти людини** (нове) | `data/play_data_human/` | ні |
| Скомпільовані бінарники додатку | `release/` | так (оновлює CI автоматично) |

Дані self-play й людських партій зберігаються в окремих каталогах
навмисно — джерело даних видно з того, ЯКА це папка, а не з поля
всередині файлу. Журнал (`.json`) пише запис на КОЖЕН цикл, і
промоутнутий, і відхилений — не лише вдалі спроби.
...
Клонуй репо, тоді одна команда з кореня репо:
...
[`release/run-windows.bat`](release/run-windows.bat), винесені в
корінь репо, щоб їх було легше знайти одразу після клонування.
...
в [`release/README.md`](release/README.md). Хочеш перевірити все
середовище вручну, окремо від запуску, — `python src/smoke_test.py`
(з `src/`).
